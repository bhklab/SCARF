...
def calc_metrics(ground_truth: sitk.Image, prediction: sitk.Image) -> pd.DataFrame:
    """
    Calculate various evaluation metrics for organ-at-risk (OAR) segmentation predictions.

    Args:
        ground_truth (sitk.Image): The ground truth segmentation image (SimpleITK format).
        prediction (sitk.Image): The predicted segmentation image (SimpleITK format).

    Returns:
        pd.DataFrame: A DataFrame containing the calculated metrics for each OAR, including:
                      - VolDice: Dice coefficient
                      - 95HD: 95th percentile Hausdorff distance
                      - SurfDist: Average surface distance
                      - APL: Added path length
                      - FNPL: False negative path length
                      - FNV: False negative volume
    """
